...
Frank Rosenblatt představil perceptron, jeden z nejznámějších raných modelů učící se neuronové sítě.

Zásadní změna byla v tom, že některé parametry systému nebyly pouze ručně naprogramované.

Systém je mohl **upravovat podle příkladů**.
...
predikce
    ↓
porovnání se správným výsledkem
    ↓
úprava parametrů
```

To už začíná připomínat princip Machine Learningu, který používáme dnes.

Perceptron byl velmi omezený a očekávání kolem něj později výrazně předběhla jeho skutečné schopnosti.

Ale myšlenka byla zásadní:

> **Nemusíme stroji napsat všechna pravidla. Některá pravidla se může naučit z dat.**

Tato věta je jedním z nejdůležitějších přechodů v celé historii AI.
...
V roce 2017 byla publikována práce *Attention Is All You Need* a s ní architektura **Transformer**.

Její význam budeme detailněji řešit v kapitole 3.

Pro historii stačí pochopit jednu věc:

Transformer umožnil velmi efektivně pracovat se vztahy mezi částmi sekvence a dobře škálovat trénování na moderním hardware.

Před Transformerem byla práce s dlouhými textovými sekvencemi často založená na recurrent neural networks.

Transformer otevřel cestu k mnohem větším modelům.

```text
Transformer
     ↓
lepší škálování práce s jazykem
     ↓
větší pre-trained modely
     ↓
LLM
```

Bez Transformeru by dnešní generace jazykových modelů pravděpodobně nevypadala tak, jak ji známe.
